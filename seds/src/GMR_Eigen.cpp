...
bool GaussianMixture::saveMuAndSigma(const char fileMu[], \
			const char fileSigma[], MatrixXd outData, MatrixXd outSigma[])
{
	...
	for(unsigned int i=0;i<outData.rows();i++) {
		for(unsigned int j=0;j<outData.cols();j++)
			Mu_file << outData(i,j) << " ";
			Mu_file << std::endl;
	for(unsigned int k=0;k<outData.cols();k++) {
		for(unsigned int j=0;j<outData.cols();j++)
